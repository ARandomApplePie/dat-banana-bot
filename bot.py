...
{message.content}
    ...
    # Level
    data = await bot.db.rank.find_one({"id": str(message.guild.id)})
    if data["enabled"]:
        ...
        if user["points"] == user["next"]:
            ...
            await message.channel.send(f"uwu **{message.author.name}** my senpai, you leveled up to level **{user['level']}**!") #Can i ask, what in the living fucking daylights did i just read on line 343.
        else:
            data["data"]["data"][str(message.author.id)] = {
    ...
    await bot.db.rank.update_one({"id": str(message.guild.id)}, {"$set": {"data": data, "enabled": True}}, upsert=True)
